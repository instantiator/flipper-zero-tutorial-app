...
void test_app_scene_on_enter_popup_one(void* context) {
    ...
    popup_set_timeout(app->popup, 3000); // 3s timeout
...
void test_app_scene_manager_init(TestApp* app) {
    FURI_LOG_I(TAG, "test_app_scene_manager_init");
...
void test_app_view_dispatcher_init(TestApp* app) {
    FURI_LOG_I(TAG, "test_app_view_dispatcher_init");
    app->view_dispatcher = view_dispatcher_alloc();
    view_dispatcher_enable_queue(app->view_dispatcher);
    view_dispatcher_set_event_callback_context(app->view_dispatcher, app);

    // allocate each view
    FURI_LOG_I(TAG, "test_app_view_dispatcher_init allocating views");
    app->menu = menu_alloc();
    app->popup = popup_alloc();

    // assign callback that pass events from views to the scene manager
    FURI_LOG_I(TAG, "test_app_view_dispatcher_init setting callbacks");
...
int32_t test_app_app(void* p) {
    ...
    FURI_LOG_I(TAG, "running dispatcher");
